...
// directly. The menu is portalled to <body> and anchored to the trigger via
// useAnchoredPosition, so no ancestor's transform or overflow can shift or clip
// it, and it stays glued to the button on scroll/resize.
...
export function SiteSelect({ value, onChange, options = [], placeholder, style, ariaLabel }) {
  ...
  // Mövqe yalnız açılan anda deyil, sürüşdürmə və pəncərə ölçüsü dəyişəndə də
  // yenilənir — əvvəl bir dəfə hesablanıb donurdu: modalın içini sürüşdürəndə
  // və ya ekran çevriləndə siyahı düymədən ayrı düşürdü.
  const coords = useAnchoredPosition(open, tRef, { panelHeight: 300, gap: 6, matchWidth: true });
  ...
  return (
    <div style={{ position: "relative", width: "100%" }}>
      ...
      {/* Siyahı <body>-yə portal edilir: modal kartının açılış animasiyası
          (fill-mode: both) bitəndən sonra da transform saxlayır, transformlu
          element isə `fixed` övlad üçün containing block olur — siyahı kartın
          içinə bağlanıb `overflow: hidden` ilə kəsilirdi. */}
      {open && coords && typeof document !== "undefined" && createPortal(
        <div ref={mRef} data-dialog-portal="" style={{ position: "fixed", left: coords.left, width: coords.width, top: coords.top, bottom: coords.bottom, zIndex: 200, background: "#fff", border: "1px solid #E4E6EF", borderRadius: 13, boxShadow: "0 16px 40px rgba(20,20,45,.18)", overflow: "hidden" }}>
          {searchable && (
            <div style={{ padding: "8px 10px", borderBottom: "1px solid #EEF0F4" }}>
              {/* Siyahı açılan kimi yazmağa başlamaq olsun deyə fokus axtarış
                  sahəsindədir (jsx-a11y/no-autofocus portalın içini görmür). */}
          ...
          {/* Alçaq ekranda (landşaft telefon) sabit 230px siyahı sığmırdı. */}
          <div style={{ maxHeight: "min(230px, 45dvh)", overflowY: "auto", padding: "4px 0" }}>
