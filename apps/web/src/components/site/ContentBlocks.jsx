...
function DefinitionsBlock({ block }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 11, margin: "0 0 18px" }}>
      {(block.definitions || []).map((d, j) => (
...
function HighlightBlock({ block }) {
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "flex-start", background: "var(--accent-soft)", border: "1px solid var(--accent)", borderRadius: 14, padding: "16px 18px", margin: "0 0 18px" }}>
      <span style={{ fontSize: 19, flex: "none" }}>★</span>
      <p style={{ margin: 0, fontSize: 15.5, lineHeight: 1.7, color: "#26263a", fontWeight: 600 }}>{block.body}</p>
    </div>
  );
}
...
function NoteBlock({ block }) {
  return (
    <p style={{ fontSize: 15, lineHeight: 1.7, color: "#55555f", margin: "0 0 16px", padding: "13px 16px", background: "#F7F8FC", borderRadius: 12 }}>{block.body}</p>
  );
}
...
function ParagraphBlock({ block }) {
  ...
  return (
    <div>
      {block.heading && (
        <Heading style={{ fontFamily: "'Poppins'", fontWeight: 700, fontSize: block.headingLevel === 3 ? "clamp(19px,2.3vw,24px)" : "clamp(23px,2.9vw,31px)", color: "#14141C", letterSpacing: "-.02em", margin: "0 0 14px" }}>{block.heading}</Heading>
      )}
...
export function ContentBlocks({ blocks = [] }) {
  return (
    <>
      {blocks.map((b, i) => {
        const Renderer = BLOCK_RENDERERS[b.type] || ParagraphBlock;
